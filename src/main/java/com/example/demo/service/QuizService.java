...
public class QuizService {
    ...
    public void deleteQuiz(Long id) {
        ...
        // Deleting the quiz deletes all questions and results automatically
    ...
    public boolean hasAttempted(Long quizId, String username) {
        return !resultRepository.findByQuizIdAndUsername(quizId, username).isEmpty();
    }

    public List<QuizResult> getResultsForQuiz(Long quizId) {
        // Directly fetch from quiz_results table by quiz_id
        return resultRepository.findByQuizId(quizId);
    }

 // Get all results of a quiz for the creator
    public List<QuizResultForCreatorDTO> getResultsForCreator(Long quizId) {
        // Fetch all QuizResult entities for this quiz ID
        List<QuizResult> results = resultRepository.findByQuizId(quizId);
        
        // Map each QuizResult to DTO
        return results.stream()
                .map(r -> new QuizResultForCreatorDTO(
                        r.getUsername(),
                        r.getScore(),
                        r.getTotal()
                ))
